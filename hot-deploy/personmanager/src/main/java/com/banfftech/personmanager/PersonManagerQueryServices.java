...
import com.banfftech.platformmanager.bean.ForwardLine;
import com.banfftech.platformmanager.constant.PeConstant;
import com.banfftech.platformmanager.util.AesCbcUtil;
import com.banfftech.platformmanager.util.Base64Util;
...
import com.banfftech.platformmanager.util.GZIP;
...
public class PersonManagerQueryServices {
    ...
    //
    Map<String,Object> forwardKeyMap = new HashMap<String, Object>();
    ...
    public Map<String, Object> queryMyProductStoreList(DispatchContext dctx, Map<String, Object> context) throws GenericEntityException, GenericServiceException, Exception {
        ...
        if(historyStore.size()>0){
            for(GenericValue gv : historyStore){
                ...
                rowStore.put("productStoreId",productStoreId);
                rowStore.put("storeName",storeName);
                rowStore.put("roleTypeId","CUSTOMER");
                rowStore.put("storeImage", store.getString("oldHeaderLogo"));
                rowStore.put("description",store.getString("title"));
...
//        List<GenericValue> allList=  EntityQuery.use(delegator).from("YpForwardChainFact").where().queryList();
        ...
        PagedList<GenericValue>   queryList = EntityQuery.use(delegator).from("YpForwardChainFact").
                where().distinct().queryPagedList(pageIndexInt, 2200);
...
//        }
    ...
    public static Map<String, String> queryBomPersonBaseInfo(Delegator delegator, String partyId,String partyIdFrom) throws GenericEntityException, GenericServiceException {
        ...
        if (person != null) {
            ...
            GenericValue telecomNumber = EntityUtil.getFirst(
                    EntityQuery.use(delegator).from("TelecomNumberAndPartyView").where(UtilMisc.toMap("partyId", partyId, "contactMechTypeId", "TELECOM_NUMBER")).orderBy("-fromDate").queryList());
            String tel = null;
            if (UtilValidate.isNotEmpty(telecomNumber)) {
                tel = telecomNumber.getString("contactNumber");
            }
